...
int main (int argc, char * argv [], char * env [])
{
	...
	if (!extr.extract (argv [1], argv [2]))
	{
		std::cerr << "'" << argv [0] << "'::error::could not extract from file '" << argv [1] << "' to file '" << argv [2] << "'" << std::endl;
		
		return EXIT_FAILURE;
	}
	std::cout << "'" << argv [0] << "'::success::extracted from file '" << argv [1] << "' to file '" << argv [2] << "'" << std::endl;
}
