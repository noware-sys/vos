...
int main (int argc, char * argv [], , char * env [])
{
	...
	if (argc < 3)
	{
		std::cout << "'" << argv [0] << "': usage: '" << argv [0] <<"' <store.txt> <thread id>" << std::endl;
	...
	
	{
		//noware::mach m;
		//noware::mach::cmd cli;
		noware::mach::store::loader loader;
		/*
		assert (cli.init ());
		assert (cli.enable ());
		assert (cli.start ());
		*/
		...
		noware::pause ("Press [Enter] to enqueue the store . . . ");
		
		//if (!m.enqueue (argv [1]))
		if (!loader.load_file (argv [1], argv [2]))
		{
			std::cout << "'" << argv [0] << "': error: could not load the store file" << std::endl;
	...
	//	std::cout << "'" << argv [0] << "': running" << std::endl;
		...
		*/
		...
		noware::pause ("Press [Enter] to finalize the store loader . . . ");
	}
