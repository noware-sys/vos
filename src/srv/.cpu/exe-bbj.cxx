...
bool const noware::smach::cpu::exe (/*vmach::cpu::*/bbj::instr const & instr, std::string const & thread_name)
{
	...
	std::cerr << "noware::mach::cpu::exe()::instr.operation[" << instr._oprn << "]" << std::endl;
	switch (instr._oprn)
	{
		case /*vmach::cpu::*/bbj::oprn::_null:
			// Do not perform anything.
			return true;
		case /*vmach::cpu::*/bbj::oprn::_bbj:
			...
			if (instr.src_dev == "0")
				// literal
				val = instr.src;
			else if (instr.src_dev == "1")	// reference to store
			...
			//output:
			if (instr.dest_dev == "0")
				// literal/null : do not perform anything
				return true;
			else if (instr.dest_dev == "1")	// store
