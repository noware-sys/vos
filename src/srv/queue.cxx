	...
	try
	{
		...
		switch (*msg_req_subj)
		{
			case msg::subj::empty:
				...
				break;
			case msg::subj::full:
				std::cerr << grp_dft << "::respond()::if::message[subject]==full::in scope" << std::endl;
				
				bool full_;
				full_ = full ();
				re.data [1] = zmq::message_t (&full_, sizeof full_);
				break;
			case msg::subj::size:
...
template <>
bool const/* success*/ noware::vos::srv::queue <std::string>::respond (zmq::msg & re, zmq::msg const & req, zyre_event_t const * const/* event*/, std::string const &/* event_type*/, std::string const &/* src*/, net::cast const &/* src_cast*/)
{
	std::cerr << grp_dft << "<std::string>::respond()::call" << std::endl;
	...
	try
	{
		...
		{
			case msg::subj::empty:
				std::cerr << grp_dft << "<std::string>::respond()::if::message[subject]==empty::in scope" << std::endl;
			...
			case msg::subj::full:
				...
				break;
			case msg::subj::size:
				std::cerr << grp_dft << "<std::string>::respond()::size" << std::endl;
				...
				break;
			case msg::subj::next_dequeue:
				std::cerr << grp_dft << "<std::string>::respond()::next_dequeue" << std::endl;
				...
				if (!queue.empty ())
				{
					...
					// reason: do not dequeue if the caller did not receive the element
					//queue.pop ();
					re.data [1] = zmq::message_t (element.c_str (), element.size () + 1/* for '\0'*/);
					//succ = true;
				}
				//else
				//	succ = false;
				...
				break;
			case msg::subj::next:
				std::cerr << grp_dft << "<std::string>::respond()::next" << std::endl;
			...
			case msg::subj::dequeue:
			{
				std::cerr << grp_dft << "<std::string>::respond()::dequeue" << std::endl;
			...
			case msg::subj::enqueue:
			{
				std::cerr << grp_dft << "<std::string>::respond()::enqueue" << std::endl;
				...
				{
					std::cerr << grp_dft << "<std::string>::respond()::enqueue::succ" << std::endl;
					
					val_t element;
					//req.dump ();
					std::cerr << grp_dft << "<std::string>::respond()::enqueue::cast" << std::endl;
					element = req.data.at (1).data.data <char const> ();
					std::cerr << grp_dft << "<std::string>::respond()::enqueue::push" << std::endl;
					queue.push (element);
			...
			case msg::subj::clear:
			{
				std::cerr << grp_dft << "<std::string>::respond()::clear" << std::endl;
	...
	{
		std::cerr << grp_dft << "<std::string>::respond()::exception" << std::endl;
		return false;
	}
	
	std::cerr << grp_dft << "<std::string>::respond()::return" << std::endl;
	return true;
}
