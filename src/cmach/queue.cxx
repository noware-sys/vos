...
#include "queue.hxx"
#include "dev.cxx"
#include "../mmach/queue.cxx"
#include <cln/nr.cxx>

const std::string noware::cmach::queue::grp_dft = "noware::cmach::queue";

noware::cmach::queue::queue (void)
{
	//assert (_node.join (grp_dft));
	//assert (_node.join ("noware::cmach::queue::nonfull"));
}
...
// const bool noware::cmach::queue::start (void)
// {
// 	if (!dev::start ())
// 		return false;
// 	
...
bool const noware::cmach::queue::empty (void) const
{
	...
	return std::string (anyval (zmq::msg (expression_serial), noware::mmach::queue::grp_dft)) == "1";
	//return 0;
}

/*
const bool noware::cmach::queue::empty_local (void) const
{
	return queue.empty ();
}
*/

const bool noware::cmach::queue::full (void) const
{
	return false;
}

/*
const bool noware::cmach::queue::full_local (void) const
{
	return false;
}
*/

const cln::nr noware::cmach::queue::size (void) const
{
	//noware::tree <std::string, std::string> expression;
	std::map <std::string, std::string> expression;
...
const std::string noware::cmach::queue::next (void) const
{
	//if (empty ())
	//	return "";
...
const std::string noware::cmach::queue::next_dequeue (void)
{
	//if (empty ())
	//	return "";
...
const bool noware::cmach::queue::dequeue (void)
{
	//if (empty ())
	//	return false;
...
const bool noware::cmach::queue::enqueue (const std::string & element)
{
	//if (full ())
	//	return false;
	...
	if (std::string (anyval (zmq::msg (expression_serial), noware::mmach::queue::grp_dft)) == "1")
	{
// 		// Enqueuing notice for CPU.
// 		zmq::msg notice;
// 		std::string peer_id;
// 		
...
bool const noware::cmach::queue::clear (void)
{
	//noware::tree <std::string, std::string> expression;
	std::map <std::string, std::string> expression;
...
// const bool/* success*/ noware::cmach::queue::respond (zmq::msg & msg_response, const zmq::msg & msg_request, zyre_event_t const * const event, const std::string & event_type, const std::string &/* src*/, const net::cast &/* src_cast*/)
// {
...
// 	}
// 	//std::cerr << "noware::cmach::queue::respond()::std::string(msg_request.data[1])==[" << std::string (msg_request.data [1]) << "]" << std::endl;
...
// 	
// 	//std::cerr << "noware::cmach::queue::respond()::if::message[type]==" << message ["type"] << "::else::in scope" << std::endl;
// 	
...
// 		//response ["key"] = message ["key"];
// 		
// 		//std::cerr << "noware::cmach::queue::respond()::not group[ed]" << std::endl;
// 		//response ["value"] = data.size ();
// 		//std::cerr << "noware::cmach::queue::respond()::data.size ()==[" << data.size () << ']' << std::endl;
// 		//result_tmp = 0;
// 		//result_tmp += group.second.size ();
// 		result_tmp = queue.size ();
// 		response ["value"] = result_tmp.operator std::string const ();
// 		//response ["value"] = noware::var (queue.size ());
// 		//std::cerr << "noware::cmach::queue::respond()::result_tmp==[" << result_tmp << ']' << std::endl;
// 		std::cerr << grp_dft << "::respond()::response[value]==[" << response ["value"] << ']' << std::endl;
// 	}
...
// 			if (empty_local ())
// 			{
// 				assert (_node.leave ("noware::cmach::queue::nonempty"));
// 			}
// 			
// 			assert (_node.join ("noware::cmach::queue::nonfull"));
// 			
...
// 		std::cerr << "noware::cmach::queue::respond()::_node.unicast(response,zyre_event_peer_uuid(event)==[" << (result ? "success" : "failure") << "]" << std::endl;
// 		//return result;
// 		
// 		if (result)
// 		{
...
// }
// 
// const bool/* success*/ noware::cmach::queue::respond_post (const zmq::msg & msg_response, const zmq::msg & msg_request, zyre_event_t const * const event, const std::string & event_type, const std::string &/* src*/, const net::cast &/* src_cast*/, const bool & result)
// {
// 	if (result)
// 	{
// 		std::map <std::string, std::string> message;
...
bool const/* success*/ noware::cmach::queue::search (zmq::msg & msg_result, const zmq::msg & msg_resp, const cln::nr &/* total, expected resonses count*/, const cln::nr &/* current count of peers who responded (so far)*/, const std::string &/* src*/, const net::cast &/* src_cast*/)
{
	...
	{
		std::cerr << grp_dft << "::respond()::if::message[subject]==" << resp ["subject"] << "::in scope" << std::endl;
		
		//msg_result ["type"] = "response";
		//msg_result ["subject"] = message ["subject"];
		//response ["key"] = message ["key"];
		...
		//response ["value"] = noware::var (queue.size ());
		//std::cerr << "noware::smach::queue::respond()::result_tmp==[" << result_tmp << ']' << std::endl;
		std::cerr << grp_dft << "::respond()::response[value]==[" << resp ["value"] << ']' << std::endl;
		return msg_result == "1";
	}
	...
	else if (resp ["subject"] == "next_dequeue")
	{
		...
		//std::cerr << grp_dft << "::search()::queue.front ()==[" << queue.front () << ']' << std::endl;
	...
	*/
	...
	else if (resp ["subject"] == "dequeue")
	{
		...
		std::cerr << "noware::cmach::queue::search()::result_tmp==[" << result_tmp << ']' << std::endl;
		std::cerr << "noware::cmach::queue::search()::msg_result==[" << std::string (msg_result) << ']' << std::endl;
	...
	else if (resp ["subject"] == "enqueue")
	{
		...
		if (full_local ())
		{
			assert (_node.leave ("noware::cmach::queue::nonfull"));
		}
		
		assert (_node.join ("noware::cmach::queue::nonempty"));
...
// const bool/* success*/ noware::cmach::queue::search_local (zmq::msg & msg_result, const zmq::msg & msg_req, const std::string &/* src*/, const net::cast &/* src_cast*/)
// {
// 	std::cerr << "noware::cmach::queue::search_local()::called" << std::endl;
// 	
// 	//std::cerr << "noware::cmach::queue::search_local()::message[subject]==[" << message ["subject"] << ']' << std::endl;
// 	
// 	
...
// 	//noware::tree <std::string, std::string> resp;
// 	//noware::var resp;
// 	std::cerr << "noware::cmach::queue::search_local()::req" << std::endl;
// 	//noware::var result_local;
// 	//bool result;
// 	std::cerr << "noware::cmach::queue::search_local()::result" << std::endl;
// 	
// 	//if (!req.deserialize (msg_req))
// 	if (!noware::deserialize <std::map <std::string, std::string>> (req, std::string (msg_req)))
// 	{
// 		std::cerr << "noware::cmach::queue::search_local()::deserialize::false" << std::endl;
// 		return false;
// 	}
// 	
// 	std::cerr << "noware::cmach::queue::search_local()::deserialize" << std::endl;
// 	
// 	//resp ["subject"] = req ["subject"];
// 	
// 	std::cerr << "noware::cmach::queue::search_local()::req[\"subject\"]==[" << req ["subject"] << ']' << std::endl;
// 	
...
// 		//msg_resp = result;
// 		//std::cerr << "noware::cmach::queue::search_local()::magnitude::result==[" << result << ']' << std::endl;
// 		std::cerr << "noware::cmach::queue::search_local()::[" << req ["subject"] << "]::result==[" << std::string (msg_result) << ']' << std::endl;
// 		//std::cerr << "noware::cmach::queue::search_local()::magnitude::msg_resp==[" << std::string (msg_resp) << ']' << std::endl;
// 		return false;
// 	}
...
// }
// 
// const zmq::msg/* result*/ noware::cmach::queue::aggregate (const zmq::msg & result, const zmq::msg & response, const zmq::msg & expression, const cln::nr & responses_count/* number of peers who answered*/, const std::string &/* src*/, const net::cast &/* src_cast*/)
// {
...
// 	{
// 		std::cerr << "noware::cmach::store::respond()::if::expression[subject]==" << xpr ["subject"] << "::in scope" << std::endl;
// 		std::cerr << "noware::cmach::store::respond()::if::expression[subject]==" << xpr ["subject"] << "::in scope::reponses_count==[" << responses_count << "]" << std::endl;
// 		
// 		//if (responses_count > cln::nr::natural (0))
// 		if (responses_count > 0)
// 		{
// 			std::cerr << "noware::cmach::store::respond()::if::(responses_count > cln::nr::natural (0))" << "::in scope" << std::endl;
// 			
// 			return zmq::msg ((result == "1" && response == "1") ? "1" : "0");
// 		}
// 		else
// 		{
// 			std::cerr << "noware::cmach::store::respond()::if::(responses_count > cln::nr::natural (0))" << "::else::in scope" << std::endl;
// 			
// 			return result;
// 		}
// 	}
