...
unsigned short int const noware::smach::cpu::x86_64::noten_delay_dft = 6;
std::string const noware::smach::cpu::x86_64::grp_dft = "noware::smach::cpu::x86_64";
...
*/
noware::smach::cpu::x86_64::x86_64 (void)
{
...
const bool noware::smach::cpu::x86_64::stop (void)
{
	//if (!running ())
	//	return true;
...
const bool noware::smach::cpu::x86_64::running (void) const
{
	if (_exen == nullptr || _notification == nullptr)
		return false;
...
const bool noware::smach::cpu::x86_64::start (void)
{
	//_running = true;
...
void noware::smach::cpu::x86_64::notify (const unsigned int & delay_)
{
...
// Store
const bool noware::smach::cpu::x86_64::exist (const std::string & key) const
{
...
const bool noware::smach::cpu::exist (const std::string & group, const std::string & key) const
{
	...
	return std::string (anyval (zmq::msg (expression_serial), "noware::mmach::store")) == "1";
}
...
const bool noware::smach::cpu::clear (void)
{
	...
	return anyval (zmq::msg (expression_serial), "noware::mmach::store") == "1";
}

const bool noware::smach::cpu::x86_64::clear (const std::string & group)
{
	//noware::tree <std::string, std::string> expression;
	std::map <std::string, std::string> expression;
...
const std::string/* value*/ noware::smach::cpu::x86_64::get (const std::string & group, const std::string & key) const
{
	std::map <std::string, std::string> expression;
	std::string expression_serial;
...
const std::string/* value*/ noware::smach::cpu::x86_64::get (const std::string & key) const
{
	return get (std::string (""), key);
}

const bool/* success*/ noware::smach::cpu::x86_64::set (const std::string & group, const std::string & key, const std::string & value)
{
...
const bool/* success*/ noware::smach::cpu::x86_64::set (const std::string & key, const std::string & value)
{
	return set (std::string (""), key);
}

// Queue
const cln::nr noware::smach::cpu::x86_64::size (void) const
{
	//bool result;
	std::map <std::string, std::string> expression;
	std::string expression_serial;
	...
	return std::string (anyval (zmq::msg (expression_serial), "noware::mmach::queue"));
}

const bool noware::smach::cpu::x86_64::empty (void) const
{
	//return count () <= 0;
...
const bool noware::smach::cpu::x86_64::full (void) const
{
	return false;
}

const std::string noware::smach::cpu::x86_64::next_dequeue (void)
{
	std::map <std::string, std::string> expression;
	std::string expression_serial;
	std::string inst;
...
const std::string noware::smach::cpu::x86_64::next (void) const
{
	std::string inst;
...
const bool noware::smach::cpu::x86_64::dequeue (void)
{
	if (full ())
		return false;
	...
	return std::string (anyval (zmq::msg (expression_serial), "noware::mmach::queue")) == "1";
}

const bool noware::smach::cpu::x86_64::enqueue (std::string const & inst)
{
	//return false;
...
const bool noware::smach::cpu::x86_64::cout (const std::string & value)
{
	...
	msg = xpr_serial;
	return multival (msg, "noware::smach::cmd") == "1";
...
bool const noware::smach::cpu::x86_64::cin (std::string & result, const char & delimiter)
{
	/*
	zmq::msg msg;
	std::map <std::string, std::string> xpr;
...
const bool/* success*/ noware::smach::cpu::x86_64::respond (zmq::msg &/* response*/, const zmq::msg & rxd, const zyre_event_t */* (zyre) event*/, const std::string & event_type, const std::string &/* src*/, const net::cast &/* src_cast*/)
{
...
const bool/* success*/ noware::smach::cpu::x86_64::search (zmq::msg & msg_result, const zmq::msg & msg_resp, const cln::nr &/* total, expected resonses count*/, const cln::nr &/* current count of peers who responded (so far)*/, const std::string &/* src*/, const net::cast &/* src_cast*/)
{
	...
	else if (resp ["subject"] == "size::empty")
	{
		std::cerr << "noware::smach::cpu::search()::subject==[" << resp ["subject"] << ']' << std::endl;
