...
/*	
	...
	mpi_.transmit (delimiter + ip + action_delimiter + std::to_string ((mathematics::numbers::natural) LIB::cluster::memory::action::EXISTS) + delimiter + (std::string) "0_0" + delimiter, ip);
...
bool LIB::cluster::memory::search (const key _key, value & _value, std::string & host)
{
	...
	{
		host = LIB::network::ip::addresses () [network_device].to_string ();
		//host = "localhost";
...
bool LIB::cluster::memory::set (key _key, value _value)
{
	...
	if (search (_key, val, host) && host != local_address)
	{
		...
		//mpi_.tcp.address = host;
		if (mpi_.transmit (delimiter + local_address + delimiter + std::to_string ((mathematics::numbers::natural) action::SET) + delimiter + _key + delimiter + _value + delimiter, host))
		{
...
bool LIB::cluster::memory::unset (const key & _key)
{
	...
	else
	{
		...
		if (search (_key, _value, host) && host != local_address)
			return mpi_.transmit (delimiter + local_address + delimiter + std::to_string ((mathematics::numbers::natural) action::UNSET) + delimiter + _key + delimiter, host);
		else
			return false;
		//if (search (_key, _value))
		//{
...
bool LIB::cluster::memory::rename (const key & current_key, const key & new_key)
{
	...
	else
	{
		...
		if (search (current_key, _value, host) && host != local_address)
			return mpi_.transmit (delimiter + local_address + delimiter + std::to_string ((mathematics::numbers::natural) action::RENAME) + delimiter + current_key + delimiter + new_key + delimiter, host);
		else
			return false;
		//if (search (_key, _value))
		//{
		...
		switch (_action)
		{
			...
			case action::GET:	// The remote host asked us for the data's value.
				...
				if (variables.exists (data_local))	// We must have the data in order to reply with its value.
				{
					...
					//mpi_.tcp.address = host;
					if (mpi_.transmit (delimiter + (LIB::network::ip::addresses () [0].to_string ()) + delimiter + variables [data_local] + delimiter, host))
					{
						std::cout << "\t\t\t" << "Succeeded." << std::endl;
					}
					else
		...
		switch ((action) std::stoi (action_))
		{
			...
			case action::EXISTS:
				...
				if (variables.exists (data_local))
				{
					std::cout << "\t\t\t" << "Found." << std::endl;
					std::cout << "\t\t\t" << "Transmitting..." << std::endl;
					if (mpi_.transmit (delimiter + (LIB::network::ip::addresses () [0].to_string ()) + action_delimiter + std::to_string ((mathematics::numbers::natural) action::ANSWER) + delimiter + variables [data_local] + delimiter + data_local + delimiter, host))
