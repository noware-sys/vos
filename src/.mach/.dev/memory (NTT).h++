...
namespace LIB
{
	namespace machine
	{
		namespace resources
		{
			...
			class memory : public resource, /*virtual */LIB::NTT <value, key>
			{
				public:
					//typedef std::string value, key;
					...
					const bool rename (const key &, const key &);
					...
					memory & operator [] (const key &);
					const memory & operator [] (const key &) const;
					...
					class research
					{
						public:
							...
							// List of machines determining which have responded and which have not.
				...
				*/
				protected:
					...
					LIB::NAME_A <research, mathematics::numbers::natural> searches;
					...
					LIB::NTT <value, key> variables;	// The container.
				...
				public:
					...
					const memory & get (const key &) const;
					const bool set (const key &, const value &);
					
					const bool unset (const key &);
					const bool move (const key &/* old*/, const key &/* new*/);
