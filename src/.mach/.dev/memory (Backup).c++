...
const bool LIB::machine::resources::memory::start (void)
{
	if (! started)
	{
		//mpi_.receive_async (& LIB::machine::resources::memory::act_direct);
...
const bool LIB::machine::resources::memory::stop (void)
{
	if (started)
	{
		LIB::entity <> addr;
		...
		//mpi_.receive_async (& LIB::machine::resources::memory::act_direct);
		// mpi_.receive_async_stop (boost::bind (boost::mem_fn (& LIB::machine::resources::memory::act_direct), this, _1));
		//mpi_.receive_async-stop ();
		//mpi_.listen_async (& LIB::machine::resources::memory::act_broadcast);
		// mpi_.listen_async_stop (boost::bind (boost::mem_fn (& LIB::machine::resources::memory::act_broadcast), this, _1));
		//mpi_.listen_async_stop ();
...
bool LIB::machine::resources::memory::refresh_local_address (void)
{
	return refresh_local_address (network_device);
}

bool LIB::machine::resources::memory::refresh_local_address (const LIB::mathematics::numbers::natural & device)
{
	LIB::NAME_A <LIB::network::ip::address, LIB::mathematics::numbers::natural> addresses = LIB::network::ip::addresses ();
	
	if (addresses.exist (device))
	{
...
std::string LIB::machine::resources::memory::get_local_address (void)
{
	return _local_address;
}

std::string LIB::machine::resources::memory::refresh_and_get_local_address (void)
{
...
std::string LIB::machine::resources::memory::refresh_and_get_local_address (const LIB::mathematics::numbers::natural & device)
{
...
bool LIB::machine::resources::memory::search (const key _key, value & _value, std::string & host)
{
	//std::string msg;
	
	if (variables.exist (_key))
	{
	...
	else if (mpi_.broadcast (delimiter + _local_address + action_delimiter + std::to_string ((unsigned short int) action::EXISTS) + delimiter + _key + delimiter))
	{
		...
		timers [_key] -> async_wait (boost::bind (& LIB::machine::resources::memory::dummy, this));
...
bool LIB::machine::resources::memory::search (const key _key)
{
	value _value;
	std::string host;
...
LIB::machine::resources::memory::value LIB::machine::resources::memory::get (key _key)
{
/*
...
bool LIB::machine::resources::memory::set (key _key, value _value)
{
...
bool LIB::machine::resources::memory::unset (const key & _key)
{
	if (variables.exist (_key))
	{
	...
	else
	{
		...
		if (search (_key, _value, host) && host != _local_address)
			return mpi_.transmit (delimiter + _local_address + delimiter + std::to_string ((unsigned short int) action::UNSET) + delimiter + _key + delimiter, (LIB::entity <>) host);
		else
			return false;
		//if (search (_key, _value))
		//{
...
bool LIB::machine::resources::memory::rename (const key & current_key, const key & new_key)
{
	if (variables.exist (current_key))
	{
	...
	else
	{
		...
		if (search (current_key, _value, host) && host != _local_address)
			return mpi_.transmit (delimiter + _local_address + delimiter + std::to_string ((unsigned short int) action::RENAME) + delimiter + current_key + delimiter + new_key + delimiter, (LIB::entity <>) host);
		else
			return false;
		//if (search (_key, _value))
		//{
