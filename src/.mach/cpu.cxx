...
const unsigned int noware::mach::cpu::noten_delay_dft = 5;
...
const bool noware::mach::cpu::start (void)
{
	...
	if (_notification == nullptr)
	{
		_notification = new boost::thread (boost::bind (boost::mem_fn (&noware::mach::cpu::notify), this, noten_delay_dft));
...
const std::string noware::mach::cpu::next_dequeue (void)
{
	std::map <std::string, std::string> expression;
	std::string expression_serial;
	std::string inst;
...
const std::string noware::mach::cpu::next (void) const
{
	std::string inst;
...
const bool noware::mach::cpu::enqueue (std::string const & inst)
{
	//return false;
