		...
		while (!empty ())
		{
			...
			if (!inst.null ())
			{
				...
				if (index.operator const unsigned long int () > index_max.operator const unsigned long int ())
				{
					...
					//std::cerr << "[" << boost::this_thread::get_id () << "] noware::mach::cpu::exe::restart(reset)::thread_id[" << thread_id << "]" << std::endl;
			...
			{
				std::cerr << "[" << boost::this_thread::get_id () << "] noware::mach::cpu::next_dequeue()::null instruction" << std::endl;
			}
		}
